...
def getDataMatrix(years):
    ...
    df_dm = pd.concat([df_reg_season,df_ncaa_tourny])
    df_sp_dm = df_dm.loc[df_dm['Season'].isin(years)]
    df_sp = df_reg_season.loc[df_reg_season['Season'].isin(years)]
    ...
    data_matrix = df_sp_dm["Season"].to_dict()
    ...
    df_ncaa_tourny = pd.read_csv('data/NCAATourneyDetailedResults.csv')
    df_ncaa_tourny_sp = df_ncaa_tourny.loc[df_ncaa_tourny['Season'].isin(years)]
    for row in df_ncaa_tourny_sp.itertuples():
        data_matrix[row.Index] = {'year': data_matrix[row.Index]}
        if row.LTeamID < row.WTeamID:
            #glicko
            w_rating, l_rating = glicko[row.WTeamID].getRating(), glicko[row.LTeamID].getRating()
...
result.to_csv('{}dataMatrixTest.csv'.format(year))
